...
public class JSCheckStrategy implements CheckStrategy {
        ...
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                result.addViolation(new Violation("SYSTEM",
                        "JavaScript checker returned status " + response.statusCode(), 0, "WARNING"));
            }

        } catch (Exception e) {
            result.addViolation(new Violation("SYSTEM",
                    "JavaScript checker service unavailable - skipping ESLint analysis", 0, "WARNING"));
        }
    ...
    private void parseViolations(String responseBody, CheckResult result) {
        try {
            ...
            result.addViolation(new Violation("SYSTEM",
                    "Failed to parse JavaScript checker response", 0, "WARNING"));
        }
    }
}
